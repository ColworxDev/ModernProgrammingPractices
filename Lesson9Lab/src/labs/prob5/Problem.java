...
public class Problem {
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		...
		//		.map(c -> c.getZip()).sorted().distinct().collect(Collectors.toList());
		return list.stream().filter(Customer::test)
				.map(Customer::getZip).sorted().distinct().collect(Collectors.toList());
	}
